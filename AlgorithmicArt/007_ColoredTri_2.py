...
turtle, screen = getTurtleAndScreen("Triangles",width,height,moveWorld=True)

# Make the background gray. Notice that we do not see any gray
# because the checkerboard covers the whole screen.
background_c = random.randint(0,9)
screen.bgcolor(colors[background_c])

for row in range (0,8):
    for col in range(0,8):
        # Move to the lower left corner of the cell.
        turtle.penup()
        turtle.goto(col*cellSize + 10,row*cellSize)
        turtle.pendown()

        # color = (row+col)%8
        color = random.randint(0,9)
        if color == background_c:
