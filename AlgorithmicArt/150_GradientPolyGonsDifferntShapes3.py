...
def drawGradientPolygon(turtle, x, y, length, number, sides, color1, color2):
    OriginalPen = turtle.pensize()
    turtle.pensize(1)
    turtle.penup()
    for i in range(0, number):
        ...
        turtle.color(color)
        turtle.penup()
        turtle.goto(x + length,y - r)
        turtle.setheading(0)
        turtle.pendown()
        # xc, yc = turtle.pos()
        turtle.begin_fill()
        turtle.circle(r, steps=sides)
        turtle.end_fill()
        turtle.pensize(OriginalPen)
...
def Draw():
    ...
    title = "Polygons Different Shapes"
    ...
    for i in range(0,amount):
        ...
        x = random.randint(0,width - rad)
        y = random.randint(0,height-rad)
        ...
        turtle.penup()
        # drawGradientSquare(turtle, x + 10, y, newRad*2, number, color1, color2)
        drawGradientPolygon(turtle, x ,y + rad,rad,number,sides, color2, color1)
        turtle.penup()
        turtle.color()
        # colors = randomPalette()
