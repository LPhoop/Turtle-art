...
def ScreenSquares(turtle, x, y, width, height, minDim=20):
    if width - x > 40 and height - y > 40:
        if random.randint(0, 1) == 0: # horizontal left
            ...
            turtle.pendown()
            turtle.goto(x, y)
            ScreenSquares(turtle, x, y, width, new_y)
            ScreenSquares(turtle, x, new_y, width, height)
        else: # vetical left
            new_x = random.randint(x + minDim, width - minDim)
            turtle.goto(x, new_x)
            turtle.pendown()
            turtle.goto(x, y)
            ScreenSquares(turtle, x, y, new_x, height)
            ScreenSquares(turtle, new_x, y, width, height)
    else:
        None
...
def Draw():
    ...
    ScreenSquares(turtle,x, y, width,height, 20)
