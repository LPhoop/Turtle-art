...
def Draw():
    ...
    title = "Wobbly Gradient Triangles and Squares grid Different Palettes"
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            if (color2==color1):
                while (color2 == color1):
                    color1 = random.choice(colors)
            drawWobblyGradientSquare(turtle, x, y, cellsize, number, color1, color2)
            drawWobblyGradientTriangle(turtle, x, y, cellsize, number, color1, color2)
            # drawGradientCircle(turtle, x,y,radius,number, bg, color2)
            turtle.color()
            colors = randomPalette()
